...
public class ArenaConfig {
    ...
    if (customConfigFile.exists()) {
      ...
      if (section != null) {
        for (String arenaName : section.getKeys(false)) {
          ...
          try {
            lobbysign =
                (Sign) getSLoc(arenadat.getString(path + ".lobbysign")).getBlock().getState();
          } catch (Exception e) {
          ...
          try {
            bound =
                new Bound(
                    arenadat.getString(path + ".bound.world"),
                    BC(arenaName, "x"),
                    BC(arenaName, "y"),
                    BC(arenaName, "z"),
                    BC(arenaName, "x2"),
          ...
          if (world.getDifficulty() == Difficulty.PEACEFUL) {
            Util.warning(
                "Difficulty in world '%s' for arena '%s' is set to PEACEFUL...",
                world.getName(), arenaName);
            Util.warning(
                "This can have negative effects on the game, please consider raising the"
                    + " difficulty.");
          }

          KitManager kit = plugin.getItemStackManager().setGameKits(arenaName, arenadat);
          if (kit != null) game.setKitManager(kit);
          ...
          if (!arenadat.getStringList(path + ".bonus").isEmpty()) {
            ...
            Util.log(
                bonusItems.size()
                    + " Random bonus items have been loaded for arena: &b"
                    + arenaName);
          }
          ...
          try {
            ...
            Util.warning(
                "Failed to setup exit location for arena '%s', defaulting to spawn location of"
                    + " world '%s'",
  ...
  public Location getLocFromString(String s) {
    String[] h = s.split(":");
    return new Location(
        Bukkit.getServer().getWorld(h[0]),
        Integer.parseInt(h[1]) + 0.5,
        Integer.parseInt(h[2]),
        Integer.parseInt(h[3]) + 0.5,
