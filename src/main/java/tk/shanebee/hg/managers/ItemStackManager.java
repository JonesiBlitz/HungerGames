...
public class ItemStackManager {
  ...
  private void kitCreator(Configuration config, KitManager kit, @Nullable String gameName) {
    ...
    for (String path : config.getConfigurationSection(gameName + "kits").getKeys(false)) {
      try {
        ...
        ItemStack ches =
            getItem(config.getString(gameName + "kits." + path + ".chestplate"), false);
        ...
        kit.addKit(
            path,
            new KitEntry(stack.toArray(new ItemStack[0]), helm, boot, ches, leg, perm, potions));
      } catch (Exception e) {
        Util.log("-------------------------------------------");
        Util.warning(
            "Unable to load kit "
                + gameName
                + path
                + "! (for a more detailed message enable 'debug' in config and reload)");
        Util.log("-------------------------------------------");
        Util.debug(e);
      }
    }
  ...
  public ItemStack getItem(String args, boolean isStackable) {
    ...
    for (String s : split) {
      ...
      if (s.startsWith("enchant:")) {
      ...
      } else if ((s.startsWith("potion:") || s.startsWith("potion-type:"))
        ...
        if (nbtApi != null) itemMeta = nbtApi.getItemWithNBT(item, s).getItemMeta();
