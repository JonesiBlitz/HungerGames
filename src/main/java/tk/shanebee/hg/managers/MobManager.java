 ...
 *
 * <p>Get an instance of this class with {@link Game#getMobManager()}
 */
...
public class MobManager {
  ...
  private void loadMobs() {
    ...
    for (String time : Arrays.asList("day", "night")) {
      if (config.getConfigurationSection("mobs." + time + "." + game.getGameArenaData().getName())
          != null) {
      ...
      for (String key : section.getKeys(false)) {
        ...
        if (getSection(key) != null) {
          MobEntry entry;
          // MYTHIC MOB
          if (Objects.requireNonNull(getString(key, "type")).startsWith("MM:")
              && HG.getPlugin().getMmMobManager() != null) {
          ...
          for (int i = 1; i <= chance; i++) {
            if (time.equalsIgnoreCase("day")) dayMobs.add(entry);
            else nightMobs.add(entry);
          }
        }
      }
    }
  ...
  private String getString(String key, String section) {
    if (getSection(key).isSet(section)) return getSection(key).getString(section);
    else return null;
  }

  private int getInt(String key, String section) {
    if (getSection(key).isSet(section)) return getSection(key).getInt(section);
    else return 1;
  }
