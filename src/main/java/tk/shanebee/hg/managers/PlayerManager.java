 ...
 *
 * <p>You can get an instance of this from <b>{@link HG#getPlayerManager()}</b>
 */
...
public class PlayerManager {
  ...
  public PlayerData getSpectatorData(UUID uuid) {
    if (hasSpectatorData(uuid)) return spectatorMap.get(uuid);
    return null;
  }
   ...
   *
   * <p>This will first check if a player is playing in a game, then check if they are spectating a
   * game. <br>
   * If you would like specific data use {@link #getPlayerData(Player)} or {@link
   * #getSpectatorData(Player)}
   *
   * @param player Player to get data for
   * @return PlayerData from player, null if player is not in a game
   */
   ...
   * <p>This will first check if a player is playing in a game, then check if they are spectating a
   * game. <br>
   * If you would like specific data use {@link #getPlayerData(UUID)} or {@link
   * #getSpectatorData(UUID)}
   *
   * @param uuid UUID of player to get data for
   * @return PlayerData from player, null if player is not in a game
   */
  ...
  public PlayerData getData(UUID uuid) {
    ...
    else if (hasSpectatorData(uuid)) return getSpectatorData(uuid);
    return null;
  }
   ...
   *
   * <p>This should mainly be used <b>internally only</b>
   *
  ...
  public Game getGame(UUID uuid) {
    if (hasPlayerData(uuid)) return getPlayerData(uuid).getGame();
    else if (hasSpectatorData(uuid)) return getSpectatorData(uuid).getGame();
    else return null;
  }
}
