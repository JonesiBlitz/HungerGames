    ...
    if (args.length == 1) {
      ...
      for (String name : plugin.getCommands().keySet()) {
        if (StringUtil.startsWithIgnoreCase(name, args[0])) {
          if (sender.hasPermission("hg." + name)) matches.add(name);
        }
      }
      return matches;
      ...
      if (args[0].equalsIgnoreCase("team") && sender instanceof Player) {
        ...
        if (args.length == 2) {
          if (args[0].equalsIgnoreCase("setexit")
              && StringUtil.startsWithIgnoreCase("all", args[1])) {
        ...
        if (args.length == 2) {
          ...
          if (playerManager.hasPlayerData(((Player) sender).getUniqueId())) {
            game =
                Objects.requireNonNull(playerManager.getPlayerData(((Player) sender).getUniqueId()))
                    .getGame();
          }
          if (game != null) {
        ...
        switch (args.length) {
          ...
          case 5:
            ...
            break;
          case 6:
            if (StringUtil.startsWithIgnoreCase("<cost>", args[5])) matchesCreate.add("<cost>");
            break;
        }
