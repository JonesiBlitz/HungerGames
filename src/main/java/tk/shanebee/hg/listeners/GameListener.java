...
public class GameListener implements Listener {
  ...
  private boolean hasTotem(Player player) {
    ...
    if (inv.getItemInMainHand() != null
        && inv.getItemInMainHand().getType() == Material.TOTEM_OF_UNDYING) return true;
    return inv.getItemInOffHand() != null
        && inv.getItemInOffHand().getType() == Material.TOTEM_OF_UNDYING;
  }

  private void processDeath(
      Player player, Game game, Entity damager, EntityDamageEvent.DamageCause cause) {
    dropInv(player);
    player.setHealth(20);
    Bukkit.getScheduler()
        .runTaskLater(
            ...
            () -> {
              ...
              if (damager instanceof Player) {
                ...
                deathString = killManager.getKillString(player.getName(), damager);
                if (killManager.isShotByPlayer(damager)
                    && killManager.getShooter(damager) != player) {
              ...
              PlayerDeathEvent playerDeathEvent =
                  new PlayerDeathEvent(player, Collections.emptyList(), 0, deathString);
  ...
  private void onSprint(FoodLevelChangeEvent event) {
    ...
    if (playerManager.hasPlayerData(player)) {
      Status status =
          Objects.requireNonNull(playerManager.getPlayerData(player))
              .getGame()
              .getGameArenaData()
              .getStatus();
            ...
            if (e instanceof Player) {
              ...
              Util.scm(
                  ...
                  lang.track_nearest
                      ...
                      .replace(
                          "<location>", getDirection(p.getLocation().getBlock(), l.getBlock())));
              i.setItemMeta(im);
              p.updateInventory();
              return;
            }
  ...
  private void onChestUse(PlayerInteractEvent event) {
    ...
    if (event.getAction() == Action.RIGHT_CLICK_BLOCK && playerManager.hasPlayerData(player)) {
      ...
      assert block != null;
      PlayerData pd = playerManager.getPlayerData(player);
      if (gameManager.getGame(player.getLocation()).getGameArenaData().getStatus()
              == Status.COUNTDOWN
          || gameManager.getGame(player.getLocation()).getGameArenaData().getStatus()
              == Status.WAITING) event.setCancelled(true);
      if (block.getType() == Material.CHEST) {
        assert pd != null;
        Bukkit.getServer()
            .getPluginManager()
            .callEvent(new ChestOpenEvent(pd.getGame(), block, false));
      } else if (BlockUtils.isBonusBlock(block)) {
        assert pd != null;
        Bukkit.getServer()
            .getPluginManager()
            .callEvent(new ChestOpenEvent(pd.getGame(), block, true));
      }
    }
  }
  ...
  private boolean isSpectatorCompass(PlayerInteractEvent event) {
    ...
    return item.getItemMeta() != null
        && item.getItemMeta()
            .getDisplayName()
            .equalsIgnoreCase(Util.getColString(lang.spectator_compass));
  }

  private void handleSpectatorCompass(Player player) {
    GamePlayerData gamePlayerData =
        Objects.requireNonNull(playerManager.getSpectatorData(player))
            .getGame()
            .getGamePlayerData();
  ...
  private void onInteract(PlayerInteractEvent event) {
    ...
    if (playerManager.hasSpectatorData(player)) {
      ...
      if (isSpectatorCompass(event)) {
        ...
        && playerManager.hasPlayerData(player)) { // Player is in game
      Status status =
          Objects.requireNonNull(playerManager.getPlayerData(player))
              .getGame()
              .getGameArenaData()
        ...
        if (event.getItem() != null)
          if (event.getItem().getType().equals(Material.getMaterial(Config.leaveitemtype))) {
            playerManager.getPlayerData(player).getGame().getGamePlayerData().leave(player, false);
              ...
              .getItem()
              .getType()
              .equals(Material.getMaterial(Config.forcestartitem))) {
          ...
          if (game == null) {
            ...
            if (player.getInventory().getItemInMainHand().getType() == Material.AIR) {
              ...
              if (HG.getParty().hasParty(player)) {
                // player is in party
                if (HG.getParty().isOwner(player)
                    && ((game.getGamePlayerData().getPlayers().size()
                            + HG.getParty().partySize(player))
                        <= game.getGameArenaData().getMaxPlayers())) { // player is owner join party
                  Bukkit.getScheduler()
                ...
                Bukkit.getScheduler()
                    .scheduleSyncDelayedTask(
                        plugin, () -> game.getGamePlayerData().join(player), 2);
            } else {
      ...
      if (player.getInventory().getItemInMainHand().getType().equals(Material.STICK)
          && playerManager.hasPlayerData(player)) {
  ...
  private void onInventoryClick(InventoryClickEvent e) {
    ...
    if (status != Status.RUNNING && status != Status.BEGINNING) {
      if (Objects.equals(
          Objects.requireNonNull(e.getClickedInventory().getItem(e.getSlot())).getType(),
          Material.getMaterial(Config.leaveitemtype))) {
        Util.clearInv(player);
        game.getGamePlayerData().leave(player, false);
      } else if (Objects.equals(
          Objects.requireNonNull(e.getClickedInventory().getItem(e.getSlot())).getType(),
          Material.getMaterial(Config.forcestartitem))) {
        Util.clearInv(player);
        game.startFreeRoam();
      }
    }
  ...
  private void handleBucketEvent(PlayerBucketEvent event, boolean fill) {
    ...
    final boolean WATER =
        event.getBucket() == Material.WATER_BUCKET
            && (Config.blocks.contains("WATER") || Config.blocks.contains("ALL"));
    final boolean LAVA =
        event.getBucket() == Material.LAVA_BUCKET
            && (Config.blocks.contains("LAVA") || Config.blocks.contains("ALL"));
  ...
  private boolean isChest(Block block) {
    if (block.getType() == Material.CHEST
        ...
        || block.getState() instanceof Shulker) {
      return true;
    }
  ...
  private void onBlockExplode(BlockExplodeEvent event) {
    if (gameManager.isInRegion(event.getBlock().getLocation())) {
      GameBlockData gameBlockData =
          gameManager.getGame(event.getBlock().getLocation()).getGameBlockData();
      for (Block block : event.blockList()) {
        gameBlockData.recordBlockBreak(block);
      }
      event.setYield(0);
      ...
      event
          ...
          .setMetadata(
              ...
              new FixedMetadataValue(
                  plugin, entity.getMetadata("death-message").get(0).asString()));
    }
    if (entity instanceof Player && playerManager.hasPlayerData(entity.getUniqueId())) {
      event
          .getProjectile()
          .setMetadata("shooter", new FixedMetadataValue(plugin, entity.getName()));
    }
  }
  ...
  private void onChat(AsyncPlayerChatEvent event) {
    if (!Config.spectateChat) {
      ...
      if (playerManager.hasSpectatorData(spectator)) {
        for (UUID uuid :
            Objects.requireNonNull(playerManager.getSpectatorData(spectator))
                .getGame()
                .getGamePlayerData()
                .getPlayers()) {
    ...
    for (Game game : plugin.getGames()) {
      ...
      if (gameArenaData.isInRegion(location) && gameArenaData.getStatus() == Status.RUNNING) {
        if (event.getCause() == PlayerTeleportEvent.TeleportCause.ENDER_PEARL
            && !game.getGamePlayerData().getPlayers().contains(player.getUniqueId())
            && !game.getGamePlayerData().getSpectators().contains(player.getUniqueId())) {
          event.setCancelled(true);
        }
      }
    }
