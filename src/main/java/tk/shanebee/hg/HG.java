...
public class HG extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    metrics1.addCustomChart(
        new SimplePie(
            "partyplugin",
            () -> party.getClass().getName().replaceAll("tk.shanebee.hg.util.", "")));
    loadPlugin(true);
  }
  ...
  public void loadPlugin(boolean load) {
    ...
    // Bukkit.getLogger().info("Your server version is: " + getServer().getVersion() + "     Your
    // Java Version is " + System.getProperty("java.version"));
    String Version = System.getProperty("java.version");
    if (Version.contains("1.8")) {
      Bukkit.getLogger()
          .info("Your Java Version is " + Version + "     This plugin requires Java 11 or higher");
      Bukkit.getPluginManager().disablePlugin(this);
      return;
    }
    ...
    if (version < 11) {
      ...
      Bukkit.getLogger()
          .info(
              "Your Java Version is "
                  + System.getProperty("java.version")
                  + " This plugin is compatible with your version but we suggest you update to Java"
                  + " 17 or higher");
    } else if (version >= 17) {
      Bukkit.getLogger()
          .info(
              "Your Java Version is "
                  + System.getProperty("java.version")
                  + " This plugin is compatible with your version");
    }
    ...
    if (Bukkit.getPluginManager().isPluginEnabled("mcMMO")) {
      if (Util.classExists("com.gmail.nossr50.events.skills.secondaryabilities.SubSkillEvent")) {
        ...
        Util.log(
            "&7mcMMO classic found. HungerGames does not support mcMMO classic, mcMMO hooks"
                + " &cdisabled");
      }
    } else {
    ...
    if (Config.allowParty)
      if (getServer().getPluginManager().isPluginEnabled("Spigot-Party-API-PAF")) {
        getLogger()
            .info(
                "Hook into Spigot Party API for Party and Friends Extended (by Simonsator)"
                    + " support!");
        party = new PAFBungee();
    ...
    Util.log(
        "HungerGames has been &aenabled&7 in &b%.2f seconds&7!",
    ...
    for (UUID u : ps) {
      ...
      if (p != null) {
        ...
        if (playerManager.hasPlayerData(u)) {
          Objects.requireNonNull(playerManager.getPlayerData(u))
              .getGame()
              .getGamePlayerData()
              .leave(p, false);
        ...
        if (playerManager.hasSpectatorData(u)) {
          Objects.requireNonNull(playerManager.getSpectatorData(u))
              .getGame()
              .getGamePlayerData()
              .leaveSpectate(p);
          playerManager.removePlayerData(u);
