...
public class TimerTask implements Runnable {
  ...
  public TimerTask(Game g, int time) {
    ...
    g.getGamePlayerData()
        .getPlayers()
        .forEach(uuid -> Objects.requireNonNull(Bukkit.getPlayer(uuid)).setInvulnerable(false));
  ...
  public void run() {
    ...
    if (Config.borderEnabled && remainingtime == borderCountdownStart) {
      ...
      game.getGamePlayerData()
          .msgAll(lang.game_border_closing.replace("<seconds>", String.valueOf(closingIn)));
    }

    if (gameArenaData.getChestRefillTime() > 0
        && remainingtime == gameArenaData.getChestRefillTime()) {
      ...
      if (!Config.bossbar) {
        ...
        if (minutes != 0) {
          if (asd == 0) {
            ...
            game.getGamePlayerData()
                .msgAll(
                    end_minsec.replace("<minutes>", "" + minutes).replace("<seconds>", "" + asd));
          }
        } else {
