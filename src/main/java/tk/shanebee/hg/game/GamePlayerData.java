...
public class GamePlayerData extends Data {
  ...
  private void kitHelp(Player player) {
    ...
    for (int i = 0; i < 20; ++i) Util.scm(player, " ");
  ...
  public void respawnAll() {
    for (UUID u : players) {
      ...
      if (p != null) p.teleport(pickSpawn());
    }
  }
  ...
  public void msgAll(String message) {
    ...
    for (UUID u : allPlayers) {
      Player p = Bukkit.getPlayer(u);
      if (p != null) Util.scm(p, message);
    }
  }

  /**
   * Sends a message to all players/spectators <b>Includes players who have died and left the game.
  ...
  public void msgAllPlayers(String message) {
    ...
    for (UUID u : allPlayers) {
      Player p = Bukkit.getPlayer(u);
      if (p != null) Util.scm(p, lang.prefix + message);
    }
  }
  ...
  boolean containsPlayer(Location location) {
    ...
    for (UUID u : players) {
      Player p = Bukkit.getPlayer(u);
      if (p != null && p.getLocation().getBlock().equals(location.getBlock())) return true;
    }
    return false;
  }
  ...
  boolean vaultCheck(Player player) {
    if (Config.economy) {
      ...
      if (Vault.economy.getBalance(player) >= cost) {
        ...
        Util.scm(
            player, lang.prefix + lang.cmd_join_no_money.replace("<cost>", String.valueOf(cost)));
        return false;
      }
    }
    return true;
          ...
          .thenAccept(
              a -> {
                ...
                if (Config.enableleaveitem) {
                  ItemStack leaveitem =
                      new ItemStack(
                          Objects.requireNonNull(Material.getMaterial(Config.leaveitemtype)), 1);
                ...
                if (Config.enableforcestartitem && player.hasPermission("hg.forcestart")) {
                  ItemStack start =
                      new ItemStack(
                          Objects.requireNonNull(Material.getMaterial(Config.forcestartitem)), 1);
                ...
                if (players.size() >= game.gameArenaData.minPlayers
                    && (status == Status.WAITING || status == Status.READY)) {
                  game.startPreGame();
                } else if (status == Status.WAITING) {
                  String broadcast =
                      lang.player_joined_game
                              .replace("<arena>", gameArenaData.getName())
                              .replace("<player>", player.getName())
                          + (gameArenaData.minPlayers - players.size() <= 0
                              ? "!"
                              : ":"
                                  + lang.players_to_start.replace(
                                      "<amount>",
                                      String.valueOf((gameArenaData.minPlayers - players.size()))));
  ...
  public void leave(Player player, Boolean death) {
    ...
    if (death) {
      if (Config.spectateEnabled && Config.spectateOnDeath && !game.isGameOver()) {
        ...
        player.sendTitle(
            game.gameArenaData.getName(),
            Util.getColString(lang.spectator_start_title),
            10,
            100,
            10);
  ...
  void exit(Player player, @Nullable Location exitLocation) {
    ...
    if (gameArenaData.getStatus() == Status.RUNNING) game.getGameBarData().removePlayer(player);
    Location loc;
  ...
  public void spectate(Player spectator) {
    ...
    if (Config.spectateFly) spectator.setAllowFlight(true);
  ...
  public void leaveSpectate(Player spectator) {
    ...
    if (Config.spectateFly) {
      ...
      if (mode == GameMode.SURVIVAL || mode == GameMode.ADVENTURE) spectator.setAllowFlight(false);
    }
    if (Config.spectateHide) revealPlayer(spectator);
