...
public class Game {
   ...
   * Create a new game
   *
   * <p>Internally used when loading from config on server start
   *
   ...
   * Create a new game
   *
   * <p>Internally used when creating a game with the <b>/hg create</b> command
   *
   * @param name Name of this game
   * @param bound Bounding region of this game
   * @param timer Length of the game (in seconds)
   ...
   */
  public Game(
      String name, Bound bound, int timer, int minPlayers, int maxPlayers, int roam, int cost) {
    this.plugin = HG.getPlugin();
    this.gameArenaData =
        new GameArenaData(this, name, bound, timer, minPlayers, maxPlayers, roam, cost);
   ...
   * Stop the game
   *
   * @param death Whether the game stopped after the result of a death (false = no winnings payed
   *     out)
   */
  public void stop(Boolean death) {
        ...
        if (Config.giveReward) {
          ...
          if (!Config.rewardMessages.isEmpty()) {
            for (String msg : Config.rewardMessages) {
              if (!msg.equalsIgnoreCase("none")) Util.scm(p, msg.replace("<player>", p.getName()));
            }
          }
          if (Config.cash != 0) {
    ...
    if (death) {
      String broadcast =
          lang.player_won.replace("<arena>", gameArenaData.name).replace("<winner>", winner);
  ...
  void updateAfterDeath(Player player, boolean death) {
    ...
    if (status == Status.RUNNING || status == Status.BEGINNING || status == Status.COUNTDOWN) {
      ...
      gamePlayerData.msgAll(
          lang.player_left_game
                  .replace("<arena>", gameArenaData.getName())
                  .replace("<player>", player.getName())
              + (gameArenaData.minPlayers - gamePlayerData.players.size() <= 0
                  ? "!"
                  : ": "
                      + lang.players_to_start.replace(
                          "<amount>",
                          String.valueOf(
                              (gameArenaData.minPlayers - gamePlayerData.players.size())))));
    }
    gameBlockData.updateLobbyBlock();
    gameArenaData.updateBoards();
  }
