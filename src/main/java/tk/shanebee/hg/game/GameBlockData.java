...
public class GameBlockData extends Data {
   ...
   *
   * <p>This is not recommended to use as it forces all blocks to rollback at once, which can cause
   * heavy amounts of lag.
   */
  ...
  void updateLobbyBlock() {
    ...
    sign3.setLine(
        ...
        ChatColor.BOLD
            + ""
            + game.getGamePlayerData().players.size()
            + "/"
            + game.gameArenaData.maxPlayers);
    sign2.update(true);
    sign3.update(true);
  }
        ...
        sign1.setLine(
            ...
            Util.getColString(
                HG.getPlugin()
                    .getLang()
                    .lobby_sign_cost
                    .replace("<cost>", String.valueOf(game.gameArenaData.cost))));
