...
public class GameBarData extends Data {
  ...
  public void createBossbar(int time) {
    ...
    int sec = (time % 60);
    String title =
        this.title.replace("<min>", String.valueOf(min)).replace("<sec>", String.valueOf(sec));
   ...
   *
   * <p>Will update the time of the current {@link BossBar} if one is available
   *
  ...
  public void bossbarUpdate(int remaining) {
    ...
    if (remain <= 0.5 && remain >= 0.2) bar.setColor(BarColor.YELLOW);
    if (remain < 0.2) bar.setColor(BarColor.RED);
  }

  /**
   * Clear the bar
   *
   * <p>Will remove all players and delete the current {@link BossBar}
   */
   ...
   *
   * <p>Will remove the player from the current {@link BossBar} if one is available
   *
   ...
   *
   * <p>Will add the player to the current {@link BossBar} if one is available
   *
