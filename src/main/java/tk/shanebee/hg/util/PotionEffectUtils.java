...
public enum PotionEffectUtils {
   ...
   *
   * <p><b>Format:</b> <br>
   * POTION_EFFECT_TYPE:int(duration):int(amplifier)
   *
  ...
  public static PotionEffect getPotionEffect(String data) {
    ...
    if (potionData.length == 3) {
      ...
      if (type == null) {
        potionWarning(
            "Potion effect type not found: &c" + potionData[0].toUpperCase() + " &ein: &b" + data);
        return null;
  ...
  public static void deprecationWarning(String data) {
    if (data.contains("potion:")) {
      Util.warning(
          "&c'potion:'&e has been changed to &a'potion-type:'&e please update your configs. Found:"
              + " &7"
              + data);
    }
  }
