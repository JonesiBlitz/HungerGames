...
public class BlockUtils {
  ...
  private static ImmutableSet.Builder<Material> setup(List<String> materialStrings) {
      ...
      if (!materialString.contains("tag")) {
        for (Material material : Material.values()) {
          if (materialString.equalsIgnoreCase("ALL")
              || material.toString().equalsIgnoreCase(materialString)) {
            materialBuilder.add(material);
          }
          // This is deprecated (on Nov 10/2020) and scheduled for removal in the future (use tags
          // instead)
          else if (materialString.equalsIgnoreCase("SHULKER_BOX")
              && material.toString().contains("SHULKER_BOX")
              && !materialString.contains("tag")) {
            materialBuilder.add(material);
            warnShulker = true;
          }
        }
    ...
    if (warnShulker && hasShulkerTag) {
      Util.warning(
          "It appears you are using 'SHULKER_BOX' in your config (for either bonus blocks or"
              + " breakable blocks)");
