...
public enum PotionTypeUtils {
   ...
   *
   * <p><b>Formats:</b> <br>
   * POTION-TYPE (optional start with 'LONG_' or 'STRONG_') <br>
  ...
  public static PotionData getPotionData(String data) {
    ...
    if (potionData.length == 1) {
      ...
      if (potionType == null) {
        Util.warning(
            "Potion base type not found: &c"
                + potionData[0].toUpperCase(Locale.ROOT)
                + " &ein: &b"
                + data);
      ...
      if (potionType == null) {
        ...
        potionTypeWarning(
            "Not a valid boolean: &c"
                + potionData[1].toUpperCase(Locale.ROOT)
                + " &ein: &b"
                + data);
        return null;
        ...
        potionTypeWarning(
            "Not a valid boolean: &c"
                + potionData[2].toUpperCase(Locale.ROOT)
                + " &ein: &b"
                + data);
        return null;
      }
